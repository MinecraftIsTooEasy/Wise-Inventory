...
public class WheelMoving {
    public static void onRender(Slot mouseOver) {
        WheelMovingMode mode = WiseInventoryConfig.WheelMoving.getEnumValue();
        if (mode == WheelMovingMode.NONE) return;

        int wheelStatus = Mouse.getDWheel();
        if (wheelStatus == 0) return;
        boolean scrollDown = wheelStatus < 0;
        if (mode == WheelMovingMode.INVERT) scrollDown = !scrollDown;
